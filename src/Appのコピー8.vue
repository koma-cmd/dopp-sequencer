...
const deleteSelection = () => {
  ...
  clearSelection();
  if (tutorialStep.value === 6 && tutorialDuplicateDone.value) advanceTutorial();
};
...
const startPaintMode = (color) => { paintColor.value = color; isPaintMode.value = true; };
...
// targetsは配列で複数要素を同時にハイライト可能
const tutorialSteps = [
  { title: null,            message: 'DOPP Sequencer へようこそ。',                                                           type: 'next',   targets: [] },
  { title: null,            message: '今から15分間で簡単な作曲体験をします。',                                                 type: 'next',   targets: [] },
  { title: '素材パレット',     message: '音（A〜H）を右側のエリアにドラッグ＆ドロップしてください。',                             type: 'action', targets: ['.sidebar', '.sequencer-wrapper'] },
  { title: 'カラーパレット',   message: '色を選んでブロックをクリックすると色を塗れます。塗り終わったら「適用終了」を押してください。', type: 'action', targets: ['.sidebar-color-section', '.sequencer-wrapper'] },
  { title: '作業スペース',    message: 'ブロックをドラッグして順番を入れ替えてみましょう。',                                     type: 'action', targets: ['.music-block','.sequencer-wrapper'] },
  { title: 'セレクトボックス',  message: 'セレクトボックスで音を変更してみましょう。',                                           type: 'action', targets: ['.selectbox','.sequencer-wrapper'] },
  { title: '選択ボタン',     message: '始点→終点を選択して範囲を作り、「複製」→選択した複製を「選択削除」してみましょう。',       type: 'action', targets: ['.sequencer-wrapper', '.controls_group'] },
  { title: '再生と停止',     message: '再生・停止・保存ボタンの使い方です。確認したら次へ進んでください。',                       type: 'next',   targets: ['.controls_play'] },
  { title: 'やり直し',      message: '「戻る」を1回、「進む」を1回押してみましょう。',                                          type: 'action', targets: ['.controls_undo-redo'] },
  { title: 'ヘルプ',       message: '操作方法を忘れた時はいつでも確認できます。完了を押してください。',                           type: 'done',   targets: ['.controls_help'] },
];
...
// ハイライト処理の世代管理
let tutorialHighlightToken = 0;

// 指定ステップのtargetsにハイライトを付与する
const applyTutorialHighlight = (step) => {
  const currentToken = ++tutorialHighlightToken;
  ...
  nextTick(() => {
    // 古い nextTick 処理なら何もしない
    if (currentToken !== tutorialHighlightToken) return;

    // すでに別ステップへ進んでいたら何もしない
...
<template>
  <div class="app-layout">
    ...
    <!-- =====================================================
         自作チュートリアルポップアップ
         tutorial-overlay: pointer-events:none で背景操作を透過
         tutorial-popover: z-index:960 でハイライト(950)より上 → 常に明るい
         ===================================================== -->
    <div v-if="isTutorialActive" class="tutorial-overlay">
      <div class="tutorial-popover">
        ...
        <div class="tutorial-footer">
          ...
          <button v-if="tutorialSteps[tutorialStep].type === 'next'" class="tutorial-btn" @click="advanceTutorial">次へ →</button>
          <button v-else-if="tutorialSteps[tutorialStep].type === 'done'" class="tutorial-btn" @click="endTutorial">完了</button>
      ...
      <div class="header">
        ...
        <div class="controls">
          ...
          <div class="controls_undo-redo">
            <button @click="undo" class="btn-undo-redo" :disabled="undoStack.length === 0">↩ 戻る</button>
            <button @click="redo" class="btn-undo-redo" :disabled="redoStack.length === 0">↪ 進む</button>
          </div>
...
/* =====================================================
   自作チュートリアル
   tutorial-overlay: pointer-events:none → 背景の操作を妨げない
   tutorial-popover: z-index:960 → ハイライト(950)より上なので常に明るい
   ===================================================== */
.tutorial-overlay {
  position: fixed;
  top: 0; left: 0;
...
/* =====================================================
   チュートリアルハイライト
   - box-shadowの巨大なspreadで背景を暗くし対象要素だけ明るく見せる
   - transform / position は変更しないのでレイアウトに影響しない
   - z-index: 950（ポップアップ960より低いので常に暗くならない）
   ===================================================== */
.tutorial-highlight {
  z-index: 950 !important;
  box-shadow: 0 0 0 4px rgba(52, 152, 219, 0.8),
              0 0 0 9999px rgba(0, 0, 0, 0.65) !important;
  border-radius: 4px;
